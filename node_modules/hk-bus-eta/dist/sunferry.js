...
function fetchEtas({ route, seq, freq, holidays, serviceDayMap, }) {
    if (seq === 1)
    ...
    return fetch(`https://www.sunferry.com.hk/eta/?route=${route}`, {
        ...
        if (!data)
            return [];
        return data.map(({ depart_time, rmk_en, rmk_tc, route_tc, route_en }) => {
            const date = new Date();
        ...
        const now = new Date();
        return (0, utils_1.getUpcomingFerry)({
            holidays,
            serviceDayMap,
            freq,
            date: new Date(),
...
//# sourceMappingURL=sunferry.js.map