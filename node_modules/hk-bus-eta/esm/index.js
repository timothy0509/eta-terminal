...
export async function fetchEtas({ route, stops, bound, dest, freq, seq, serviceType, co, nlbId, gtfsId, stopList, language, holidays, serviceDayMap, }) {
    try {
        let _etas = [];
        for (const company_id of co) {
            if (company_id === "kmb" && stops.kmb) {
                ...
                _etas = _etas.concat(await fortuneferry({
                    stops,
                    seq,
                    stopList,
                    freq,
...
export async function fetchRouteUpdatedAt(route) {
