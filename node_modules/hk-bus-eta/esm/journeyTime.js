...
async function fetchEstJourneyTimeBasedOnHistoricalData({ route, startSeq, endSeq, }) {
    const requests = [];
    for (let i = startSeq; i < endSeq; ++i) {
        const start = Object.values(route.stops)[0][startSeq];
        const end = Object.values(route.stops)[0][endSeq];
        const day = parseInt(formatInTimeZone(new Date(), "Asia/Hong_Kong", "i"), 10) - 1;
...
async function fetchEstJourneyTimeBasedOnHistoricalAvgData({ route, startSeq, endSeq, }) {
    const requests = [];
    for (let i = startSeq; i < endSeq; ++i) {
        const start = Object.values(route.stops)[0][startSeq];
        const end = Object.values(route.stops)[0][endSeq];
 ...
 */
export async function fetchEstJourneyTime({ route, stopList, startSeq, endSeq, batchSize = 4, signal, }) {
    ...
    for (let i = startSeq; i < endSeq; ++i) {
        ...
        const seconds = await Promise.all(payloads.map(([key, payload, { startSeq, endSeq }]) => {
