...
export default function fetchEtas({ stopId, route, dest, }) {
    return fetch(`https://rt.data.gov.hk/v1/transport/mtr/lrt/getSchedule?station_id=${stopId.slice(2)}`, {
        cache: isSafari ? "default" : "no-store",
    })
        .then((response) => response.json())
        .then(({ platform_list }) => {
        if (platform_list.every(({ end_service_status }) => end_service_status)) {
            return [
                {
                    eta: null,
                    remark: {
        ...
        return platform_list.reduce((acc, { route_list, platform_id }) => {
            return [
                ...acc,
                // route_list is null when there are no ETAs available
                ...(route_list !== null && route_list !== void 0 ? route_list : [])
                    .filter(({ route_no, dest_ch, dest_en, stop }) => route === route_no &&
                    (dest_ch === dest.zh || dest_en.includes("Circular")) &&
                    stop === 0)
                    .map(({ time_en, train_length }) => {
                    let waitTime = 0;
