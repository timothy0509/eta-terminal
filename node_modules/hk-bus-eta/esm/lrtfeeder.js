...
export default function fetchEtas({ stopId, route, language, }) {
    return fetch(`https://rt.data.gov.hk/v1/transport/mtr/bus/getSchedule`, {
        ...
        .then(({ busStop }) => busStop
        .filter(({ busStopId }) => busStopId === stopId)
        .reduce((ret, { bus: buses }) => [
        ...ret,
        ...buses.reduce((etas, bus) => {
            // hackily use +8 hours and use UTC hour to resolve timezone issue
            const etaDate = new Date(Date.now() +
                parseInt(bus.arrivalTimeInSecond === "108000"
                    ? bus.departureTimeInSecond
                    : bus.arrivalTimeInSecond, 10) *
                    1000 +
                8 * 3600000);
            return [
                ...etas,
...
//# sourceMappingURL=lrtfeeder.js.map